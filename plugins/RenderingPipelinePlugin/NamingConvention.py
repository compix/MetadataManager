...
def extractNameFromNamingConvention(namingConvention: str, documentWithSettings: dict):
    ...
    for c in namingConvention:
        if c == '[':
            ...
            if value == None:
                raise RuntimeError(f'The key {curKey} of naming convention {namingConvention} is not defined.')
            name += value
            curKey = ''
...
class NamingConvention(object):
    ...
    def getRenderingFilename(self, documentWithSettings: dict):
        return os.path.join(documentWithSettings.get(PipelineKeys.PostFolder), self.getRenderingRelPath(documentWithSettings)) + f'.{documentWithSettings.get(PipelineKeys.RenderingExtension)}'
